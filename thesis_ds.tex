...
This thesis presents an autonomous indoor mapping system that integrates real-time feature-based SLAM with feature-based SVD submap stitching. The system uses an Extended Kalman Filter (EKF) as the state estimator, fusing wheel odometry with geometric landmark observations to maintain a joint estimate of robot pose and map. Global drift correction is performed via Procrustes SVD alignment of shared wall landmarks between submaps. A separate navigation module provides frontier-based exploration with RRT* path planning and Pure Pursuit path tracking, enabling the robot to explore unknown environments autonomously. The system is implemented in Python using ROS 2 Jazzy as the middleware framework, with the TurtleBot3 Waffle Pi as the robot platform and Gazebo Harmonic for simulation.
...
\begin{enumerate}
	...
	\item \textbf{Submap Integration and Drift Correction:} At an interval of 50 scans, a point cloud based submap is generated using features is transferred to the \texttt{SubmapStitcher}. This module performs voxel-downsampling and aligns the local submap against the global map using feature-based correspondence: shared wall landmark IDs provide matched point pairs, and SVD is used to compute the optimal rigid transformation. The resulting drift correction (pose adjustment and covariance) is fed back into the EKF to maintain global consistency.
\end{enumerate}
...
\begin{equation}
	f(\mathbf{x}_{t-1}, \mathbf{u}_t) = \begin{bmatrix}
		x_{t-1} + v_x \Delta t \cos(\theta_{t-1}) - v_y \Delta t \sin(\theta_{t-1}) \\
		y_{t-1} + v_x \Delta t \sin(\theta_{t-1}) + v_y \Delta t \cos(\theta_{t-1}) \\
...
\subsection{Compute Optimal Rotation}
...
\subsection{Compute Optimal Translation}
...
\subsection{Convex Hull Computation: QuickHull Algorithm}


Given a finite set of points $\mathcal{P} = \{p_1, p_2, \ldots, p_n\}$ in Euclidean space $\mathbb{R}^d$, the \textbf{convex hull} is to find the smallest convex set $\mathcal{H} \subseteq \mathbb{R}^d$ that contains all points in $\mathcal{P}$.

QuickHull~\cite{barber1996quickhull} follows a divide-and-conquer algorithm. 
For a 2D point set $\mathcal{P}$, the algorithm starts 
by finding the two most extreme points using the $x$ values ($p_{\min}$ and 
$p_{\max}$). These points are guaranteed to be on the convex hull boundary.

\paragraph{Point Classification}
The line segment $\overline{p_{\min}p_{\max}}$ divides the plane into two half-planes. For any point $p = (p_x, p_y) \in \mathcal{P}$, are partitioned into upper and lower sets using the signed distance to the line passing through $p_{\min}$ and $p_{\max}$ :

\begin{equation}
	d(p, p_{\min}, p_{\max}) = \frac{(p_{\max,y} - p_{\min,y})p_x - (p_{\max,x} - p_{\min,x})p_y + p_{\max,x}p_{\min,y} - p_{\max,y}p_{\min,x}}{\sqrt{(p_{\max,x} - p_{\min,x})^2 + (p_{\max,y} - p_{\min,y})^2}}
\end{equation}


\begin{align}
	\mathcal{P}_{\text{upper}} &= \{p \in \mathcal{P} : d(p, p_{\min}, p_{\max}) > 0\} \\
	\mathcal{P}_{\text{lower}} &= \{p \in \mathcal{P} : d(p, p_{\min}, p_{\max}) < 0\}
\end{align}

Points with $d = 0$ lie on the line and can be discarded from further consideration.

\paragraph{Complete Algorithm}
The complete convex hull is obtained by:

\begin{equation}
	\mathcal{H} = \{p_{\min}\} \cup \text{FindHull}(\mathcal{P}_{\text{upper}}, p_{\min}, p_{\max}) \cup \{p_{\max}\} \cup \text{FindHull}(\mathcal{P}_{\text{lower}}, p_{\max}, p_{\min})
\end{equation}


\subparagraph{Geometric Operations}

\paragraph{Distance to Line Segment}
For numerical stability, the \textbf{perpendicular distance} from point $p$ to the line through $A$ and $B$ is computed using the cross product:

\begin{equation}
	d(p, A, B) = \frac{|(B - A) \times (p - A)|}{\|B - A\|}
\end{equation}


\begin{equation}
	d_{\text{signed}}(p, A, B) = \frac{(B - A) \times (p - A)}{\|B - A\|}
\end{equation}

where $d_{\text{signed}} > 0$ indicates $p$ is to the left of directed segment $\overrightarrow{AB}$.

\paragraph{Triangle Containment Test}
A point $p$ is inside triangle $\triangle ABC$ if and only if it is on the same side of all three edges. Using signed distances:

\begin{equation}
	p \in \triangle ABC \iff 
	\begin{cases}
		d_{\text{signed}}(p, A, B) \cdot d_{\text{signed}}(C, A, B) \geq 0 \\
		d_{\text{signed}}(p, B, C) \cdot d_{\text{signed}}(A, B, C) \geq 0 \\
		d_{\text{signed}}(p, C, A) \cdot d_{\text{signed}}(B, C, A) \geq 0
	\end{cases}
\end{equation}
...
\subsubsection{Wall Uncertainity}
...
\subsubsection{Corner Uncertainity}
...
\subsection{Hessian Construction and Stabilization}
...
The final pose covariance is determined by scaling the inverse of the stabilized Hessian by the LiDAR measurement noise variance $\sigma^2$:
...
\subparagraph{Preprocessing and Segmentation} First, the LiDAR points are filtered to remove invalid readings and outliers. The remaining points are grouped into segments based on a distance threshold. If the gap between two consecutive points exceeds $0.5\,\text{m}$, they are assigned to different clusters. This ensures that the algorithm does not attempt to fit a single line across two separate surfaces.

\paragraph{Incremental Line Growing}
The next step extracts lines using an incremental growing approach. New points are added one by one as long as the Total Least Squares (TLS) residual remains below $0.03\,\text{m}$. By using Singular Value Decomposition (SVD), the algorithm finds the best-fit line for the cluster of points. 

\paragraph{Hessian Parameterization and Uncertainty}
Every extracted wall is stored in Hessian normal form $(\rho, \alpha)$. This form is compact and prevents mathematical issues when lines are vertical. 

The uncertainty of these parameters is calculated using the Fisher Information Matrix. This calculation accounts for the number of points and their distribution along the wall. 
...
Corners are only extracted from \textit{adjacent} segments in the LiDAR scan order. This restriction prevents the algorithm from creating "ghost" corners between walls that are spatially close but structurally unrelated.

A corner is detected if the acute angle between two adjacent walls exceeds a threshold of $45^\circ$. This criteria allows the system to identify L-shaped corners and T-junctions while ignoring small noise-induced fluctuations on flat walls. To find the exact corner position $\mathbf{c} = [x_c, y_c]^\top$, the system solves the following linear system representing the intersection of two lines in Hessian form:
\begin{equation}
	\begin{bmatrix} \cos\alpha_a & \sin\alpha_a \\ \cos\alpha_b & \sin\alpha_b \end{bmatrix} \begin{bmatrix} x_c \\ y_c \end{bmatrix} = \begin{bmatrix} \rho_a \\ \rho_b \end{bmatrix}
\end{equation}

The algorithm checks the determinant of the matrix $\mathbf{A}$ to avoid singularity.

\subsection{Landmark Initialization and State Augmentation}
...
Features are transformed from the Robot Frame ($R$) to the Map Frame ($M$) using the current robot pose estimate $(x_r, y_r, \theta_r)$. 
...
\begin{align}
	\alpha_m &= \text{arctan2}(\alpha_r + \theta_r) \\
...
\subsubsection{Nearest-Neighbor Selection}
...
1. Prediction: The algorithm computes the predicted state ($\hat{\mathbf{x}}_k^-$) through a nonlinear odometry function and calculates the expected state covariance ($\mathbf{P}_k^-$) by passing the state covariance through the system's linearized form. 
...
\paragraph{Process Noise Parameter Selection}

\textbf{Design Decision:} The base process noise variances are set to:
\begin{itemize}
	\item $\sigma_d^2 = 0.01$ (distance noise coefficient)
	\item $\sigma_\theta^2 = 0.02$ (rotation noise coefficient)
\end{itemize}
...
This conditioning pipeline is applied to $\mathbf{P}$ immediately after the prediction step and again after the Joseph form update.
...
\begin{itemize}
	\item \textbf{Observation Timeout:} If a landmark is not re-observed within 25 consecutive scans, it is considered out of the sensor's range or an outlier and is removed.
...
The alignment of the current submap $\mathcal{S}$ to the global map $\mathcal{M}$ is performed using the Iterative Closest Point (ICP) algorithm with a point-to-point estimation objective. Given that the submap is initialized via the EKF pose, the transformation $\mathbf{T}$ starts from identity $\mathbf{I}$.

The optimization objective seeks to minimize the Euclidean distance between matched pairs:
\begin{equation}
	\mathbf{T}^* = \arg\min_{\mathbf{T}} \sum_{i=1}^{N_c} \| \mathbf{T}\mathbf{p}_i - \mathbf{q}_i \|^2
\end{equation}
where $\mathbf{q}_i$ is the nearest neighbor in $\mathcal{M}$ within a search radius of $\delta_{max} = 0.03\,\text{m}$.
...
\subsubsection{Transformation Extraction and Validation}
...
\begin{equation}
	\Delta x = T_{1,4}, \quad \Delta y = T_{2,4}, \quad \Delta \theta = \operatorname{atan2}(T_{2,1}, T_{1,1})
\end{equation}
...
In the event of rejection, the system employs a \textit{Fallback Strategy}, where the ICP correction is ignored, and the robot state is propagated solely via the EKF prediction. This ensures that transient sensor noise or feature-poor environments do not introduce discontinuities into the global map.
...
\end{equation}
where $\tau = 50$ is a normalization constant. This formulation ensures that:
\begin{itemize}
...
This confidence metric is logged for each submap and used in post-processing evaluation to identify low-confidence regions requiring re-exploration.

\subsection{Wall Endpoint Correction and Global Mapping}

\subsubsection{Motivation}
After the ICP alignment phase corrects the submap pose, the semantic features—specifically the wall landmarks—must be updated to reflect the refined coordinate frame. This process ensures geometric consistency between the registered point cloud and the high-level landmarks stored in the \textit{FeatureMap}. By updating these endpoints, the system maintains a topologically accurate map that supports reliable long-term data association.

\subsubsection{Wall Representation}
In this system, walls are parameterized using the Hessian normal form supplemented by tangential extents. A wall landmark $W$ is defined by the tuple $\{\rho, \alpha, t_{min}, t_{max}\}$, where:
\begin{itemize}
	\item $\rho$ is the perpendicular distance from the origin to the line.
	\item $\alpha$ is the orientation of the normal vector.
	\item $t_{min}, t_{max}$ are the scalar extents along the line's tangent.
\end{itemize}




\subsubsection{Endpoint Correction Algorithm}
When the ICP algorithm provides a rigid transformation consisting of a rotation matrix $\mathbf{R} \in SO(2)$ and a translation vector $\mathbf{t} \in \mathbb{R}^2$, the wall landmarks are updated through the following geometric pipeline.

\subsubsection{Transformation and Re-parameterization}
The reconstructed endpoints are transformed into the corrected global frame:
\begin{equation}
	\mathbf{p}' = \mathbf{R}\mathbf{p} + \mathbf{t}
\end{equation}

The corrected orientation $\alpha'$ is calculated by rotating the source normal by the ICP-derived heading change $\Delta\theta$. The updated perpendicular distance $\rho'$ is then computed by projecting the corrected segment centroid onto the new normal vector $\mathbf{\hat{n}}'$:
\begin{equation}
	\rho' = \left| \left( \frac{\mathbf{p}'_{start} + \mathbf{p}'_{end}}{2} \right) \cdot \mathbf{\hat{n}}' \right|
\end{equation}
To maintain the uniqueness constraint $\rho \ge 0$, if the projection result is negative, the normal vector $\mathbf{\hat{n}}'$ is inverted and $\alpha'$ is shifted by $\pi$.

\subsubsection{Tangential Coordinate Projection}
Finally, the endpoints are projected onto the corrected tangent vector $\mathbf{\hat{t}}'$ to establish the updated scalar extents:
\begin{equation}
	t'_{s} = \mathbf{p}'_{start} \cdot \mathbf{\hat{t}}', \quad t'_{e} = \mathbf{p}'_{end} \cdot \mathbf{\hat{t}}'
\end{equation}
The final extents are assigned such that $t'_{min} = \min(t'_s, t'_e)$ and $t'_{max} = \max(t'_s, t'_e)$.



\subsubsection{Global Wall Registry and Landmark Fusion}
To facilitate incremental mapping, corrected landmarks are merged into a global registry. This allows the system to aggregate partial observations of the same wall into a single contiguous feature, as described in Algorithm~\ref{alg:wall_fusion}.

\begin{algorithm}
	\caption{Global Landmark Fusion and Extent Extension}
	\label{alg:wall_fusion}
	\begin{algorithmic}[1]
		\Require Corrected wall landmark $W'$, Global Registry $\mathcal{G}$
		\Ensure Updated Global Registry $\mathcal{G}$
		\If{$W'_{id} \notin \mathcal{G}$}
		\State $\mathcal{G}[W'_{id}] \gets \{ \rho', \alpha', t'_{min}, t'_{max} \}$ \Comment{Initialize new global landmark}
		\Else
		\State $W_{g} \gets \mathcal{G}[W'_{id}]$ \Comment{Retrieve existing landmark}
		\State $\mathbf{\hat{t}}_{g} \gets [-\sin\alpha_g, \cos\alpha_g]^\top$
		\State $p_{s} \gets \mathbf{p}'_{start} \cdot \mathbf{\hat{t}}_{g}$
		\State $p_{e} \gets \mathbf{p}'_{end} \cdot \mathbf{\hat{t}}_{g}$
		\State $W_{g}.t_{min} \gets \min(W_{g}.t_{min}, p_{s}, p_{e})$ \Comment{Expand observed boundaries}
		\State $W_{g}.t_{max} \gets \max(W_{g}.t_{max}, p_{s}, p_{e})$
		\EndIf
	\end{algorithmic}
\end{algorithm}
...
\paragraph{Perception Layer:} The perception layer processes the point cloud map (generated by SLAM) to identify frontiers - boundaries between explored free space and unknown regions. This component:
...
The implementation utilizes the QuickHull algorithm~\cite{barber1996quickhull} via SciPy's \texttt{ConvexHull} function. The \texttt{scipy.spatial.ConvexHull} object provides the following attributes:
\begin{itemize}[noitemsep]
	\item \textbf{vertices}: Indices of hull vertices ordered counterclockwise in 2D
	\item \textbf{simplices}: Edge connectivity matrix
	\item \textbf{volume}: In 2D, this represents the enclosed area
\end{itemize}
...
\begin{table}[h]
	\centering
	\caption{Parameter Selection Guidelines}
	\label{tab:parameters}
	\begin{tabular}{@{}llll@{}}
		\toprule
		Parameter & Symbol & Typical Range & Purpose \\
		\midrule
		Lookahead distance & $L_d$ & 0.5 - 1.5 m & Bigger values make smoother but wider turns \\
		Smoothing factor & $\alpha_s$ & 0.3 - 0.6 & Smaller values give smoother but slower response \\
		Velocity gain & $k_v$ & 0.3 - 0.7 & Controls how much speed drops in turns \\
		Goal tolerance & $\delta_{goal}$ & 0.1 - 0.5 m & How close to get to the goal \\
		Max linear velocity & $v_{max}$ & 0.1 - 0.5 m/s & Depends on the robot \\
		Max angular velocity & $\omega_{max}$ & 0.5 - 2.0 rad/s & Depends on the robot \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{itemize}
	...
	\item \textbf{Ground Truth:} Direct access to the robot's true pose from the physics engine via the PosePublisher plugin
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{../thesis_ws/images/gazebo_world.png}
	\caption{Maze world environment showing the 30×30m arena with walls, corridors, and obstacles. The robot starts at the southwest corner.}
	\label{fig:maze_world}
\end{figure}
...
The system's performance depends critically on proper parameter tuning. This section details all key parameters and the rationale for their chosen values.

\subsection{LiDAR and Feature Extraction Parameters}
...
\begin{table}[htbp]
	...
	\begin{tabular}{lcp{8cm}}
		...
		\midrule
		\texttt{min\_range} & 0.12 m & Minimum reliable range of the LiDAR sensor \\
		\texttt{max\_range} & 3.5 m & Maximum reliable range, avoiding long-distance noise \\
		\texttt{samples\_per\_scan} & 360 & 1° angular resolution for detailed environment representation \\
		...
		\texttt{min\_line\_length} & 0.3 m & Reject short segments that may be noise or small objects \\
		\texttt{max\_line\_gap} & 0.2 m & Maximum gap to merge adjacent collinear segments \\
		\bottomrule
	\end{tabular}
\end{table}

The RANSAC threshold of 0.05m was chosen based on the expected LiDAR noise characteristics. In simulation, sensor noise is typically $\sigma \approx 0.01$-$0.02$m, so 0.05m provides a $2.5\sigma$-$5\sigma$ threshold, capturing true inliers while rejecting outliers.

\subsection{EKF-SLAM Parameters}

\begin{table}[htbp]
	\centering
	\caption{EKF-SLAM noise and association parameters}
	\label{tab:ekf_params}
	\begin{tabular}{lcp{8cm}}
		\toprule
		\textbf{Parameter} & \textbf{Value} & \textbf{Rationale} \\
		\midrule
		\multicolumn{3}{l}{\textit{Process Noise (Motion Model)}} \\
		$\sigma_{\text{dist}}$ & 0.01 & Uncertainty proportional to distance traveled \\
		$\sigma_{\text{rot}}$ & 0.02 & Uncertainty proportional to rotation (rad) \\
		\midrule
		\multicolumn{3}{l}{\textit{Measurement Noise (Line Features)}} \\
		$\sigma_\rho$ & 0.1 m & Distance-to-line measurement uncertainty \\
		$\sigma_\alpha$ & 0.1 rad & Line angle measurement uncertainty \\
		\midrule
		\multicolumn{3}{l}{\textit{Data Association}} \\
...
The chi-square gate threshold of 5.99 corresponds to the 95\% confidence level for a 2-DOF $\chi^2$ distribution, providing robust data association. Wall features additionally use geometric constraints: parallel walls must have angles within 20° and perpendicular distances within 0.5m for matching.

\subsection{Submap Generation Parameters}
...
The choice of 50 scans per submap (approximately 5 seconds of operation at 10 Hz) provides a good balance:
\begin{itemize}
	\item Sufficient local consistency: Small enough that drift within a submap is negligible
	\item Computational efficiency: Large enough to accumulate meaningful point clouds while avoiding excessive memory usage
	\item Alignment granularity: Submaps contain enough geometric structure for reliable SVD-based registration
\end{itemize}
...
\subsection{Navigation and Exploration Parameters}

\begin{table}[htbp]
	\centering
	\caption{Navigation and frontier-based exploration parameters}
	\label{tab:nav_params}
	\begin{tabular}{lcp{8cm}}
		\toprule
		\textbf{Parameter} & \textbf{Value} & \textbf{Rationale} \\
		\midrule
		\multicolumn{3}{l}{\textit{Velocity Limits}} \\
		\texttt{max\_linear\_velocity} & 0.26 m/s & TurtleBot3 platform maximum safe speed \\
		\texttt{max\_angular\_velocity} & 1.82 rad/s & Maximum turning rate \\
		\midrule
		\multicolumn{3}{l}{\textit{Reactive Obstacle Avoidance}} \\
		\texttt{emergency\_distance} & 0.5 m & Critical distance triggering emergency stop \\
		\texttt{angular\_scan\_range} & 60° & Forward-facing cone for obstacle detection \\
		\texttt{rotation\_speed} & 0.3 rad/s & Slow rotation speed when avoiding obstacles \\
		\bottomrule
	\end{tabular}
\end{table}

The emergency distance of 0.5m provides a safety margin given the robot's maximum speed (0.26 m/s) and reaction time, ensuring collision-free operation even with control delays.
...
\subsubsection{RViz Visualization}

\begin{figure}[htbp]
	\centering
	% TODO: Add RViz screenshot showing all visualization elements
	\caption{RViz visualization interface showing: (a) global occupancy map, (b) robot pose, (c) LiDAR scan, (d) detected frontiers (green markers), (e) navigation goal (red arrow), and (f) EKF path history.}
	\label{fig:rviz_overview}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.3\textwidth]{../thesis_ws/images/lidar_scan.png}
	\caption{Feature extraction results showing detected wall segments (red lines) extracted from LiDAR scan data. The robot's coordinate frame is shown at the center, with line features fitted using RANSAC representing wall boundaries in the environment.}
	\label{fig:feature_detection}
\end{figure}

\subsubsection{Local Submap Visualization}

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.75\textwidth]{../thesis_ws/images/local_submap.png}
	\caption{Current local submap showing extracted wall features accumulated over multiple LiDAR scans. The colored line segments (yellow/orange) represent detected wall landmarks, with the robot coordinate frame at the center. Features are transformed using EKF pose estimates.}
	\label{fig:local_submap}
\end{figure}

\subsubsection{Global Map Construction}

\begin{figure}[htbp]
	\centering
	% TODO: Add global map after full exploration
	\caption{Final global map constructed through submap stitching after complete exploration of the maze. The map clearly shows walls, corridors, and obstacle boundaries.}
	\label{fig:global_map}
\end{figure}

\subsubsection{Convex Hull Boundary Detection}

The convex hull represents the explored region boundary, used for frontier detection.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.75\textwidth]{../thesis_ws/images/hull_&_frontiers.png}
...
\begin{figure}[htbp]
	\centering
	% TODO: Add frontier markers visualization
	\caption{Detected frontier points (green markers) representing boundaries between known free space and unknown regions. The navigation system selects the nearest accessible frontier as the next exploration goal.}
	\label{fig:frontiers}
\end{figure}
% ==============================================================================
...
The Absolute Trajectory Error measures the deviation between the EKF-estimated pose and the ground truth from Gazebo's physics engine. We compute both position and orientation errors over the entire exploration run.

\begin{figure}[htbp]
	\centering
	% TODO: Add ATE position error over time graph
	\caption{Absolute Trajectory Error (position) over time. The plot shows the Euclidean distance between EKF pose and ground truth throughout the exploration mission.}
	\label{fig:ate_position}
\end{figure}

\begin{figure}[htbp]
	\centering
	% TODO: Add ATE orientation error over time graph
	\caption{Absolute Trajectory Error (orientation) over time. The plot shows the angular deviation between EKF heading and ground truth orientation.}
	\label{fig:ate_orientation}
\end{figure}

\subsection{Submap Confidence Evolution}
The confidence metric, based on the trace of the EKF covariance matrix, indicates the SLAM system's certainty about its submaps state estimate. 

\begin{figure}[htbp]
	\centering
	% TODO: Add submap confidence pre/post stitching graph
	\caption{EKF confidence evolution showing pre-stitching and post-stitching confidence values for each submap. The reduction in confidence after stitching demonstrates the benefit of loop closure corrections.}
...
\end{equation}
where $I_{\text{landmarks}}$ is the sum of landmark information (trace of inverse covariance) and $\tau = 10^6$ is a normalization constant.
...
\begin{itemize}
	\item \textbf{2D Planar Assumption:} System assumes flat floors and vertical walls. Fails on ramps, stairs, or tilted surfaces.
	\item \textbf{Static Environment:} Moving obstacles (people, furniture) treated as spurious landmarks, pruned via timeout. No explicit dynamic object modeling.
	\item \textbf{Geometric Features Only:} Relies on walls and corners. Fails in open spaces (e.g., atriums, parking lots) without structure.
\end{itemize}

\section{CONCLUSION}

This thesis presented a feature-based EKF-SLAM system for autonomous indoor exploration that addresses fundamental challenges in probabilistic mapping and localization. By integrating geometric feature extraction, rigorous uncertainty quantification, and drift-correcting submap alignment, the system produces both geometrically accurate and statistically consistent maps suitable for long-term autonomous navigation in structured indoor environments.
...
The Mahalanobis distance is scale-invariant. If we rescale a dimension by multiplying it by a constant, the Mahalanobis distance remains unchanged (because the covariance scales accordingly).

\subsubsection{Unit-Free}

The Mahalanobis distance is dimensionless. It measures distance in units of standard deviations rather than in the original units of the variables.
...
For a multivariate normal distribution $\mathbf{x} \sim \mathcal{N}(\boldsymbol{\mu}, \boldsymbol{\Sigma})$, the squared Mahalanobis distance follows a chi-squared distribution:
\begin{equation}
	D_M^2(\mathbf{x}) \sim \chi^2(n)
\end{equation}

where $n$ is the dimensionality. This property is useful for outlier detection and hypothesis testing.


\section{Wall Landmark Update (Hessian Normal Form)}

\subsection{Wall Representation}
A wall (represented as an infinite line) in the map frame is parameterized using the Hessian Normal Form. This representation is preferred for its minimality and lack of singularities compared to slope-intercept forms. The wall is defined by the parameter set $\mathbf{m} = [\rho_m, \alpha_m]^T$:
...
Where:
\begin{itemize}
	\item $\rho_m \in [0, \infty)$: The perpendicular distance from the map origin to the line.
	\item $\alpha_m \in [-\pi, \pi]$: The angle of the normal vector relative to the map's x-axis.
\end{itemize}



\subsection{Observation Model: Map Frame $\rightarrow$ Robot Frame}
Given the robot pose in the map frame $\mathbf{x}_r = [x_r, y_r, \theta_r]^T$ and the wall parameters $\mathbf{m} = [\rho_m, \alpha_m]^T$, the predicted observation in the robot's local frame $\mathbf{h}(\mathbf{x})$ is derived as follows:

\subsubsection{Local Wall Distance ($\rho_r$)}
The perpendicular distance from the robot to the wall is the difference between the wall's absolute distance from the origin and the robot's own projection onto the wall's normal vector:

\begin{equation}
	\rho_r = \rho_m - (x_r \cos(\alpha_m) + y_r \sin(\alpha_m))
\end{equation}

\subsubsection{Local Wall Orientation ($\alpha_r$)}
The wall's normal angle in the robot's local frame is calculated by rotating the global normal angle by the robot's current heading:

\begin{equation}
	\alpha_r = \text{atan2}(\sin(\alpha_m - \theta_r), \cos(\alpha_m - \theta_r))
\end{equation}

This formulation ensures the angular representation remains normalized within the $[-\pi, \pi]$ interval.

\subsection{Complete Measurement Model}
The non-linear observation model $h(\mathbf{x}, \mathbf{m})$ used for the measurement update (e.g., within an EKF or UKF framework) is summarized as:

\begin{equation}
	h(\mathbf{x}, \mathbf{m}) = \begin{bmatrix} 
		\hat{\rho}_r \\ \hat{\alpha}_r 
	\end{bmatrix} = \begin{bmatrix} 
		\rho_m - (x_r \cos \alpha_m + y_r \sin \alpha_m) \\ 
		\text{atan2}(\sin(\alpha_m - \theta_r), \cos(\alpha_m - \theta_r)) 
	\end{bmatrix}
\end{equation}

\subsection{Observation Jacobian for Wall Landmarks}

To linearize the observation model for use in an Extended Kalman Filter (EKF), we compute the Jacobian matrix $\mathbf{H}_{wall}$. This matrix represents the partial derivatives of the observation function $h(\mathbf{x}, \mathbf{m})$ with respect to the state vector $\mathbf{x}$. Assuming the state vector consists of the robot pose $[x_r, y_r, \theta_r]^T$ followed by $N$ wall landmarks, the Jacobian is defined as:

\begin{equation}
	\mathbf{H}_{wall} = \frac{\partial h_{wall}}{\partial \mathbf{x}} = 
	\begin{bmatrix} 
...
\subsubsection{Resulting Sparse Jacobian Structure}
The complete Jacobian for a single wall landmark measurement at index $i$ in the map state is a sparse matrix:

\begin{equation}
	\mathbf{H}_{wall} = \begin{bmatrix} 
		-\cos(\alpha_m) & -\sin(\alpha_m) & 0 & \cdots & 1 & x_r \sin(\alpha_m) - y_r \cos(\alpha_m) & \cdots \\ 
		0 & 0 & -1 & \cdots & 0 & 1 & \cdots 
	\end{bmatrix}
\end{equation}

In this structure, the first three columns correspond to the robot's pose $[x_r, y_r, \theta_r]$, and the columns starting at $i$ and $i+1$ correspond to the specific observed landmark $[\rho_m, \alpha_m]$. All other entries in the matrix remain zero, as the observation is independent of other landmarks in the map.

\section{Corner Landmark Update (Cartesian Representation)}

\subsection{Corner Representation}
Corner landmarks are stored in the map frame using Cartesian coordinates:
\begin{equation}
	\mathbf{m}_c = \begin{bmatrix} x_m \\ y_m \end{bmatrix} \in \mathbb{R}^2
\end{equation}

Unlike walls, corners are point features with finite spatial extent, making Cartesian representation natural and singularity-free.

\subsection{Observation Model: Map Frame $\rightarrow$ Robot Frame}

Given the robot pose $\mathbf{x}_r = [x_r, y_r, \theta_r]^\top$ and corner landmark $\mathbf{m}_c = [x_m, y_m]^\top$ in the map frame, the predicted observation in the robot's local frame is obtained by:

\subsubsection{Translation to Robot Origin}
The corner position is first translated relative to the robot:
\begin{equation}
	\Delta \mathbf{p} = \begin{bmatrix} \Delta x \\ \Delta y \end{bmatrix} = \begin{bmatrix} x_m - x_r \\ y_m - y_r \end{bmatrix}
\end{equation}

\subsubsection{Rotation into Local Frame}
The translated vector is rotated by $-\theta_r$ to align with the robot's local coordinate system:
\begin{equation}
	\mathbf{h}_{\text{corner}}(\mathbf{x}_r, \mathbf{m}_c) =
...
\subsection{Observation Jacobian for Corner Landmarks}

The Jacobian $\mathbf{H}_{\text{corner}} \in \mathbb{R}^{2 \times n}$ has the sparse structure:
\begin{equation}
	\mathbf{H}_{\text{corner}} =
	\begin{bmatrix}
		\frac{\partial x_L}{\partial x_r} & \frac{\partial x_L}{\partial y_r} & \frac{\partial x_L}{\partial \theta_r} & \cdots & \frac{\partial x_L}{\partial x_m} & \frac{\partial x_L}{\partial y_m} & \cdots \\
		\frac{\partial y_L}{\partial x_r} & \frac{\partial y_L}{\partial y_r} & \frac{\partial y_L}{\partial \theta_r} & \cdots & \frac{\partial y_L}{\partial x_m} & \frac{\partial y_L}{\partial y_m} & \cdots
	\end{bmatrix}
\end{equation}

\subsubsection{Derivatives with Respect to Robot Pose}
Let $c = \cos(\theta_r)$ and $s = \sin(\theta_r)$ for brevity.

\textbf{Position derivatives:}
\begin{align}
	\frac{\partial x_L}{\partial x_r} &= -c, \quad \frac{\partial x_L}{\partial y_r} = -s \\
	\frac{\partial y_L}{\partial x_r} &= s, \quad \frac{\partial y_L}{\partial y_r} = -c
\end{align}

\textbf{Heading derivatives:}
\begin{align}
	\frac{\partial x_L}{\partial \theta_r} &= \frac{\partial}{\partial \theta_r}\left[c(x_m - x_r) + s(y_m - y_r)\right] \\
	&= -s(x_m - x_r) + c(y_m - y_r) = -s\Delta x + c\Delta y \\
	\frac{\partial y_L}{\partial \theta_r} &= \frac{\partial}{\partial \theta_r}\left[-s(x_m - x_r) + c(y_m - y_r)\right] \\
	&= -c(x_m - x_r) - s(y_m - y_r) = -c\Delta x - s\Delta y
\end{align}

\subsubsection{Derivatives with Respect to Landmark Position}
\begin{align}
	\frac{\partial x_L}{\partial x_m} &= c, \quad \frac{\partial x_L}{\partial y_m} = s \\
	\frac{\partial y_L}{\partial x_m} &= -s, \quad \frac{\partial y_L}{\partial y_m} = c
\end{align}

\subsubsection{Complete Sparse Jacobian}
For a corner landmark at state index $j$ (where $\mathbf{x}[j:j+2] = [x_m, y_m]^\top$), the Jacobian is:
\begin{equation}
	\mathbf{H}_{\text{corner}} =
	\begin{bmatrix}
	...
	\end{bmatrix}
\end{equation}
where the first three columns correspond to $[x_r, y_r, \theta_r]$, columns $j$ and $j+1$ correspond to $[x_m, y_m]$, and all other entries are zero.

\subsubsection{Implementation Note}
In the code (\texttt{ekf\_slam\_feature.py}, lines 217-241), $\Delta x = x_m - x_r$ and $\Delta y = y_m - y_r$ are precomputed (as \texttt{dx, dy}). The rotation matrix uses the identities $\cos(-\theta_r) = \cos(\theta_r)$ and $\sin(-\theta_r) = -\sin(\theta_r)$, which simplifies the observation prediction. This implementation is numerically equivalent to the formulation above.
...
\subsubsection{Rotation into Local Frame}
To obtain the coordinates $(x_L, y_L)$ in the robot's local frame, the translated vector $\Delta \mathbf{p}$ must be rotated by the inverse of the robot's heading $\theta_r$:
