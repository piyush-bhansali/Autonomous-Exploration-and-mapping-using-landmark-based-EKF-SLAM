...
\begin{table}[htbp]
	...
	\begin{tabular}{lcp{8cm}}
		...
		\texttt{icp\_max\_corr\_dist} & 0.03 m & Max distance for point correspondences; limits matches to nearby points \\
...
Using 50 scans per submap balances local consistency against accumulated drift, it yields enough points to stabilize feature estimates while keeping each submap short enough that odometry error does not dominate. A 0.05 m voxel size provides effective downsampling to reduce memory and computation while preserving wall geometry; it also roughly matches typical scan spacing at common indoor ranges. The ICP minimum fitness threshold (0.3) is intentionally lenient so alignments are not rejected when overlap is limited, while still filtering out extremely poor matches
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.5\textwidth]{../thesis_ws/images/lidar_scan.png}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.75\textwidth]{../thesis_ws/images/submap_confidence.png}
	\caption{Confidence value for each submap generated}
	\label{fig:submap confidence}
\end{figure}
