...
\usepackage{subcaption}  % for >=stealth arrow style
...
\begin{titlepage}
	...
	\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.3\textwidth]{../thesis_ws/images/logo_rosso.png}
	\end{figure}

% --- Department Information ---
{\Large \textsc{University of Trento}} \\[0.2cm]
{\large \textsc{Department of Industrial Engineering}} \\[0.2cm]
{\large \textsc{Master´s degree in Mechatronics Engineering}} \\[0.4cm]
{\large Academic Year 2022--2023} \\[3cm]

% --- Thesis Title ---
{\huge \textbf{Autonomous Robot Exploration using Landmark-based EKF-SLAM}} \\[4cm]

% --- Supervisor and Student Section ---
\vfill
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lr}
	\textbf{Supervisor} & \textbf{Student} \\
	Prof. Daniele Fontanelli & Piyush Bhansali \\
	& 238881 % Replace with your ID
\end{tabular*}
...
\end{titlepage}
	...
	\begin{abstract}
		...
		\vspace{0.5em}
		\noindent
		The system is implemented as a ROS 2 node in Python, tested on TurtleBot3 in structured indoor environments. Performance is assessed using the absolute trajectory error (ATE), landmark map confidence, and point cloud quality.
				
			
		\vspace{0.5em}
		\noindent
		Keywords: SLAM, Extended Kalman Filter, Feature-Based Mapping, Landmark Extraction, ICP Alignment, RRT*, ROS2, Frontier-Based Exploration, 2D LiDAR Mapping, Cramér-Rao Lower Bound
	\end{abstract}
	...
	\subsection{State of the Art in Indoor Mapping and Autonomous Navigation}
	\label{sec:sota_indoor}
	
	Foundational SLAM work formalized the probabilistic formulation and identified the core bottlenecks of scalability, localisation, and map representation, which still structure modern system design. Classical tutorials emphasized computational complexity, loop closure, and representation choices as key constraints for practical systems, while later survey work framed the field as moving toward robust perception and long-term operation. \cite{durrant2006slam,cadena2016past}
	
	Recent systems increasingly adopt smoothing/graph-based estimation and tight multi-sensor coupling. Visual based SLAM has matured into robust, multi-session systems (e.g., ORB-SLAM3), while LiDAR-inertial frameworks (e.g., LIO-SAM) provide high-accuracy odometry and mapping with factor-graph formulations and real-time performance. Although these systems are robust and more accurate , they come with a trade off, as these are computationally and memory intensive due to large factor graphs and global optimization. They require fine tuning of parameters and may introduce latency from batch updates. Visual SLAM can fail in low‑texture or low‑light environments and is sensitive to motion blur and dynamic scenes.
		
	For indoor mapping, LiDAR-based SLAM surveys from 2023 highlight the diversity of sensor configurations and remaining challenges in robustness and deployment, while semantic mapping has become a standard extension to support task-level navigation and interaction. \cite{yue2023lidar_slam_survey} More recently, implicit representations (e.g., NeRF-based SLAM) have emerged as a promising direction for dense, photorealistic indoor reconstruction, though real-time performance and scalability remain active challenges. \cite{yang2024nerf_slam_survey}
...
This thesis focuses on implementing a feature‑based EKF‑SLAM system with explicit uncertainty modeling for wall and corner landmarks. Wall covariances are obtained from the Fisher information (via the Cramér–Rao lower bound), and corner covariances are propagated analytically through line‑intersection geometry, providing a principled estimate of map confidence. The system is evaluated in indoor environments for localization accuracy, landmark consistency, and overall mapping quality, with the aim of preserving probabilistic correctness. Compared to graph‑based alternatives, the approach is intended to be less computationally and memory intensive and to reduce latency.
...
\subsubsection{Autonomous Exploration and Navigation}
...
While landmark based EKF-SLAM and frontier exploration have been studied extensively, probabilistic based mapping using geometry-aware CRLB covariances for landmarks and point cloud based ICP drift correction is the objective of this thesis. A convex‑hull–based frontier detection algorithm which operates directly on the point cloud to generate exploration targets without grid discretization is implemented, where frontier candidates are geometrically validated and ranked for navigation.
...
At each time step, the robot receives control inputs $\mathbf{u}_t = [\delta_d, \delta_\theta]^T$ representing linear velocities and angular velocity, and sensor observations $\mathbf{z}_t$ representing laser range measurements. The sequence of poses, control inputs, and observations up to time $t$ is denoted as:
\begin{align}
...
\subsubsection{Observation Model (Feature-Based)}
The EKF update does not use raw LiDAR beams directly. Instead, each scan is processed to extract a set of geometric features (walls and corners), which are used as observations. Let the observation set at time $t$ be:
\begin{equation}
	\mathbf{z}_t = \{ \mathbf{z}_k \}_{k=1}^{M_t},
\end{equation}
where each $\mathbf{z}_k$ is either a wall measurement $\mathbf{z}_k^{\text{wall}} = [\rho_r, \alpha_r]^\top$ in the robot frame or a corner measurement $\mathbf{z}_k^{\text{corner}} = [x_r, y_r]^\top$ in the robot frame.

\paragraph{Wall Observation Model}
Given a map-frame wall landmark $\mathbf{m}_j = [\rho_m, \alpha_m]^\top$ and robot pose $\mathbf{x}_r = [x, y, \theta]^\top$, the expected measurement in the robot frame is:
\begin{equation}
	\mathbf{z}_k^{\text{wall}} = h_{\text{wall}}(\mathbf{x}_r,\mathbf{m}_j) + \mathbf{v}_k.
\end{equation}

\paragraph{Corner Observation Model}
Given a map-frame corner landmark $\mathbf{m}_j = [m_x, m_y]^\top$, the expected measurement in the robot frame is:

\begin{equation}
	\mathbf{z}_k^{\text{corner}} = h_{\text{corner}}(\mathbf{x}_r,\mathbf{m}_j) + \mathbf{v}_k.
\end{equation}

In both cases, $\mathbf{v}_k \sim \mathcal{N}(\mathbf{0}, \mathbf{R}_k)$ models feature extraction and sensing noise.
...
The Cramér–Rao Lower Bound (CRLB) establishes a theoretical lower limit on the precision of any unbiased estimator. In the multivariate case, the CRLB is expressed as a matrix inequality, which is used here to quantify the minimum achievable uncertainty for geometric features and ICP-based alignment results.

Let $\mathbf{\theta} \in \mathbb{R}^n$ represent a vector of unknown parameters observed through a noisy measurement model:
\begin{equation}
	\mathbf{z} = h(\mathbf{\theta}) + \mathbf{v}, 
\end{equation}

The covariance matrix $\mathbf{\Sigma}_{\hat{\theta}}$ of any unbiased estimator $\mathbf{\hat{\theta}}$ is bounded by the inverse of the \textit{Fisher Information Matrix} (FIM), denoted as $\mathcal{I}(\mathbf{\theta})$. This relationship is defined by the matrix inequality:
\begin{equation}
	\mathbf{\Sigma}_{\hat{\theta}} \succeq \mathcal{I}(\mathbf{\theta})^{-1}
\end{equation}
...
The Iterative Closest Point (ICP) algorithm is used for submap stitching. 
It aligns the local submap with the fixed global map by estimating the optimal 
rigid transformation between them \cite{Besl}. 

Most ICP variants use either a point-to-point or point-to-plane formulation. 
Since this work operates on 2D LiDAR-based maps, the point-to-point formulation is employed.

Let the source point cloud and target point cloud be defined as

\begin{equation}
	\mathcal{S} = \{\mathbf{s}_i\}, \quad i \in \{1, \dots, N_s\}
\end{equation}

\begin{equation}
	\mathcal{O} = \{\mathbf{o}_j\}, \quad j \in \{1, \dots, N_o\}
\end{equation}

The first step of ICP is to determine correspondences between the two point sets. 
For each source point, the closest point in the target cloud is selected using 
a nearest-neighbour search. This produces a set of matched pairs 
$(\mathbf{p}_i, \mathbf{q}_i)$.

Given these correspondences, the rigid transformation is estimated by minimising 
the point-to-point cost function

\begin{equation}
	E(\mathbf{R}, \mathbf{t}) = \sum_{i=1}^{N} 
	\left\| \mathbf{q}_i - (\mathbf{R}\mathbf{p}_i + \mathbf{t}) \right\|^2
\end{equation}

where $\mathbf{R} \in SO(2)$ is the rotation matrix and 
$\mathbf{t} \in \mathbb{R}^2$ is the translation vector.

At iteration $k$, the estimated rigid transformation is represented in 
...
\begin{equation}
	...
	\end{bmatrix},
	\quad \mathbf{T}_k \in SE(2)
\end{equation}

After $n_{\text{ICP}}$ iterations, the final transformation is obtained by 
the product of all intermediate transformations:

\begin{equation}
	\mathbf{T}_{\text{ICP}} =
	\prod_{k=1}^{n_{\text{ICP}}} \mathbf{T}_k
	\label{eq:icp_total_transform}
\end{equation}

where $n_{\text{ICP}}$ denotes the total number of ICP iterations performed.
...
After merging, segments shorter than $\ell_{\min} = 0.5$\,m are discarded.
...
\begin{equation}
	h_i(\rho, \alpha) = \rho - p_{x,i} \cos(\alpha) - p_{y,i} \sin(\alpha)
\end{equation}
...
While wall segments are represented by their hessian parameters, corner landmarks are defined by their Cartesian coordinates $(x_c, y_c)$. In this system, a corner is treated as a derived feature resulting from the intersection of two primary wall landmarks. This section details the propagation of uncertainty from the wall parameter space to the Cartesian corner space.
...
To propagate uncertainty, we compute the Jacobian of the corner position with respect to the wall parameters $\boldsymbol{\theta} = [\rho_a, \alpha_a, \rho_b, \alpha_b]^\top$.

Starting from $\mathbf{c} = \mathbf{A}^{-1} \mathbf{b}$, we apply the chain rule:
\begin{equation}
	\frac{\partial \mathbf{c}}{\partial \boldsymbol{\theta}} =
	\frac{\partial (\mathbf{A}^{-1} \mathbf{b})}{\partial \boldsymbol{\theta}}
\end{equation}


\begin{align}
	\frac{\partial \mathbf{c}}{\partial \rho_a} &= \mathbf{A}^{-1} \begin{bmatrix} 1 \\ 0 \end{bmatrix} \\
	\frac{\partial \mathbf{c}}{\partial \rho_b} &= \mathbf{A}^{-1} \begin{bmatrix} 0 \\ 1 \end{bmatrix} \\
	\frac{\partial \mathbf{c}}{\partial \alpha_a} &= -\mathbf{A}^{-1} \frac{\partial \mathbf{A}}{\partial \alpha_a} \mathbf{c} \\
	\frac{\partial \mathbf{c}}{\partial \alpha_b} &= -\mathbf{A}^{-1} \frac{\partial \mathbf{A}}{\partial \alpha_b} \mathbf{c}
\end{align}

where:
\begin{align}
	\frac{\partial \mathbf{A}}{\partial \alpha_a} &=
	\begin{bmatrix}
		-\sin \alpha_a & \cos \alpha_a \\
		0 & 0
	\end{bmatrix} \\
	\frac{\partial \mathbf{A}}{\partial \alpha_b} &=
	\begin{bmatrix}
		0 & 0 \\
		-\sin \alpha_b & \cos \alpha_b
	\end{bmatrix}
\end{align}
...
\begin{equation}
	\mathbf{J} =
	\begin{bmatrix}
		\frac{\partial \mathbf{c}}{\partial \rho_a} &
		\frac{\partial \mathbf{c}}{\partial \alpha_a} &
		\frac{\partial \mathbf{c}}{\partial \rho_b} &
		\frac{\partial \mathbf{c}}{\partial \alpha_b}
	\end{bmatrix}
	\in \mathbb{R}^{2 \times 4}
\end{equation}

\subsubsection{Covariance Propagation Formula}

Given the covariances of the two walls:
\begin{equation}
	\boldsymbol{\Sigma}_{\boldsymbol{\theta}} =
	\begin{bmatrix}
		\mathbf{C}_a & \mathbf{0} \\
		\mathbf{0} & \mathbf{C}_b
	\end{bmatrix}
	\in \mathbb{R}^{4 \times 4}
\end{equation}

where $\mathbf{C}_a$ and $\mathbf{C}_b$ are the $2 \times 2$ wall covariances.

The first-order uncertainty propagation formula~\cite{thrun2005probabilistic} states:
\begin{equation}
	\mathbf{C}_{\text{corner}} = \mathbf{J} \boldsymbol{\Sigma}_{\boldsymbol{\theta}} \mathbf{J}^\top
	\label{eq:corner_propagation}
\end{equation}
...
\subsubsection{ICP uncertainity Estimation}

The uncertainty of the Iterative Closest Point (ICP) registration is estimated using a closed-form approach derived from the method proposed by Censi\cite{censi}. This approach relates the pose covariance to the geometric structure of the environment and the sensor's measurement noise. The fundamental insight is that the covariance of the estimated transformation $\mathbf{x}$ is inversely proportional to the Hessian matrix (or Fisher Information Matrix) of the least-squares objective function.
...
For each point correspondence $(\mathbf{p}_i, \mathbf{q}_i)$, we compute the Jacobian $\mathbf{J}_i$ by taking the partial derivatives of the error function with respect to the pose parameters. The transformation is defined as:

\begin{equation}
	\mathbf{e}_i = \begin{bmatrix} \cos \theta & -\sin \theta \\ \sin \theta & \cos \theta \end{bmatrix} \begin{bmatrix} p_x \\ p_y \end{bmatrix} + \begin{bmatrix} t_x \\ t_y \end{bmatrix} - \begin{bmatrix} q_x \\ q_y \end{bmatrix} 
\end{equation}

The resulting Jacobian matrix $\mathbf{J}_i \in \mathbb{R}^{2 \times 3}$ represents the sensitivity of the error to changes in the robot's pose:
...
To ensure numerical stability in feature-poor environments, an eigen decomposition is performed: $\mathbf{H} = \mathbf{V} \mathbf{\Lambda} \mathbf{V}^\top$. The eigenvalues are clamped to a minimum threshold $\epsilon$ to prevent singularities in the inversion process:
...
\begin{equation}
	\mathbf{\Sigma}_{\mathbf{x}} = \sigma^2 \mathbf{H}_{stable}^{-1}
\end{equation}
...
Unlike classical EKF-SLAM formulations where the observation vector $z_t$ consists 
of raw range-bearing measurements $(r_j, \phi_j)$, the observation vector in this 
system consists of extracted geometric features --- specifically, wall parameters 
$(\rho_r, \alpha_r)$ in the robot frame, and corner positions $(x_r, y_r)$ in the 
robot frame. The raw LiDAR point cloud $\{(r_j, \phi_j)\}_{j=1}^{M}$ is first 
passed through the feature extraction pipeline described in previous Section, 
which applies incremental line fitting, Hessian parameterization, and corner 
intersection to produce a set of geometric observations. These extracted features 
form the measurement input $z_t$ to the EKF update step. 
...
\begin{enumerate}[noitemsep]
	...
	\item Localization errors could place the robot close to the obstacle without it realising it.
\end{enumerate}
...
The frontier candidate $\mathbf{p}$ is accepted if at least three of the five probe directions are obstacle-free.
...
After validation, frontier candidates are grouped into spatial clusters to reduce redundancy by representing nearby frontiers as single exploration targets. Density-Based Spatial Clustering of Applications with Noise (DBSCAN), a density-based clustering algorithm is used that groups points based on spatial proximity without requiring a predefined number of clusters.
...
\begin{table}[htbp]
	...
	\begin{tabular}{lcp{8cm}}
		...
		$\sigma_d^2$ & $8\times10^{-6}$ & Process noise variance for odometry distance (m$^2$) \\
		$\sigma_\theta^2$ & $2\times10^{-5}$ & Process noise variance for odometry rotation (rad$^2$) \\
		
		\bottomrule
	\end{tabular}
\end{table}

\paragraph{Process Noise Estimation from Ground Truth and Odometry}

Process noise matrix $\mathbf{Q}$ is estimated directly from ground‑truth and odometry by analyzing incremental motion errors. For each time step $t$, compute the incremental displacement and rotation from odometry and ground truth:
\begin{align}
	\Delta d_t &= \sqrt{(x_t - x_{t-1})^2 + (y_t - y_{t-1})^2}, \\
	\Delta \theta_t &= \mathrm{wrap}(\theta_t - \theta_{t-1}).
\end{align}
Define the residuals between odometry and ground truth:
\begin{align}
	e_{d,t} &= \Delta d_t^{\text{odom}} - \Delta d_t^{\text{gt}}, \\
	e_{\theta,t} &= \mathrm{wrap}\!\left(\Delta \theta_t^{\text{odom}} - \Delta \theta_t^{\text{gt}}\right).
\end{align}
Assuming zero‑mean errors, the process noise variances are estimated as:
\begin{align}
	\sigma_d^2 &= \mathrm{Var}(e_{d,t}), \\
	\sigma_\theta^2 &= \mathrm{Var}(e_{\theta,t}),
\end{align}
and the process noise covariance is set to:
\begin{equation}
	\mathbf{Q} = \mathrm{diag}(\sigma_d^2, \sigma_\theta^2).
\end{equation}
...
\subsubsection{Sensitivity Analysis via Noise-Model Perturbation}
\label{sec:sensitivity_noise}

To evaluate the robustness of the EKF-SLAM pipeline to noise-model assumptions, we conducted a sensitivity analysis by perturbing the assumed process and measurement uncertainties while keeping all other settings, datasets, and initial conditions fixed. The analysis compares three cases: (1) a baseline configuration with nominal noise parameters, (2) increased process uncertainty, and (3) increased LiDAR measurement uncertainty.

The process uncertainty is controlled by the covariance matrix $\mathbf{Q}$ in the prediction step, so increasing $\mathbf{Q}$ explicitly models a less reliable motion model and increases the predicted covariance. The measurement uncertainty is controlled by the LiDAR noise variance $\sigma^2$ (or the measurement covariance $\mathbf{R}$), which scales feature and ICP covariances (e.g., $\mathbf{C}_{\text{wall}} = \sigma^2 \mathbf{A}^{-1}$ and $\boldsymbol{\Sigma}_{\mathbf{x}} = \sigma^2 \mathbf{H}^{-1}$). Increasing $\sigma^2$ therefore decreases measurement confidence and reduces the Kalman gain, leading to more conservative updates.

Importantly, these experiments modify the \emph{assumed} noise models used by the filter rather than the raw sensor data, isolating the EKF’s sensitivity to noise-model tuning. Performance is evaluated using identical metrics (e.g., positional error and covariance evolution) to attribute changes in accuracy and consistency directly to the altered uncertainty assumptions.

\begin{enumerate}
	\item \textbf{Case 1 (Baseline):} Nominal process noise covariance $\mathbf{Q}$ and nominal LiDAR measurement noise variance $\sigma^2$ 
	\item \textbf{Case 2 (Increased Process Uncertainty):} $\mathbf{Q}$ increased relative to the baseline, while LiDAR noise $\sigma^2$ is unchanged. 
	\item \textbf{Case 3 (Increased LiDAR Uncertainty):} LiDAR measurement variance $\sigma^2$ increased relative to the baseline, while $\mathbf{Q}$ is unchanged.
\end{enumerate}


\subsubsection{Comparison between Robot estimated Pose and Ground Truth}

Across the three cases, the EKF trajectory departs progressively from ground truth. 
In the baseline case, the estimated path closely overlaps the reference. 
With increased process uncertainty, small deviations appear, particularly after the turning segment. 
With increased LiDAR uncertainty, the separation is largest and a sustained bias develops along the trajectory, consistent with reduced measurement influence and accumulating drift. as shown in figure~7.


\subparagraph{Abosulte Trajectory Error}
Absolute Trajectory Error (ATE) measures the deviation between the estimated trajectory and ground truth. It is typically summarized using the root mean square error (RMSE), which provides a single scalar for overall trajectory accuracy; lower RMSE indicates closer agreement.

\paragraph{ATE Comparison}
The baseline configuration (Case 1) yields the lowest ATE (0.14), as the EKF noise models are consistent with the nominal sensor and motion characteristics. Increasing process uncertainty (Case 2, ATE 0.35) inflates the predicted covariance and increases the Kalman gain, making the filter rely more on measurements; this typically introduces moderate error growth when measurements are imperfect, hence the observed increase. Increasing LiDAR measurement uncertainty (Case 3, ATE 0.99) reduces measurement trust and shifts the filter toward odometry prediction, causing drift to accumulate and producing the largest trajectory error. The monotonic ATE increase across cases is therefore consistent with the EKF’s dependence on the assumed noise balance between prediction and measurement.
 
The ATE achieved in case 1 of 0.14\,m, is competitive with common ROS~2 SLAM baselines; comparative studies with SLAM Toolbox and Google Cartographer in indoor simulation report ATE values of 0.13 and 0.21\,m respectively.\cite{Ince}

\begin{figure}[htbp]
	\centering
	\begin{subfigure}[t]{0.32\textwidth}
		\includegraphics[width=\linewidth]{../thesis_ws/images/ekf_vs_groundtruth.png}
		\caption{Baseline process and lidar noise}
	\end{subfigure}
	\begin{subfigure}[t]{0.32\textwidth}
		\includegraphics[width=\linewidth]{../thesis_ws/images/ekf_vs_groundtruth_odom.png}
		\caption{With Increased Process Uncertainity}
	\end{subfigure}
	\begin{subfigure}[t]{0.32\textwidth}
		\includegraphics[width=\linewidth]{../thesis_ws/images/ekf_vs_groundtruth_lidar.png}
		\caption{With Increased Lidar uncertainity}
	\end{subfigure}
	\caption{EKF Position vs Ground Truth of the robot in three different uncertainity condition}
	\label{fig:EKFvsGT}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\begin{subfigure}[t]{0.32\textwidth}
		\includegraphics[width=\linewidth]{../thesis_ws/images/uncertainity_in_robot_position.png}
		\caption{Baseline process and lidar noise}
	\end{subfigure}
	\begin{subfigure}[t]{0.32\textwidth}
		\includegraphics[width=\linewidth]{../thesis_ws/images/uncertainity_in_robot_position_odom.png}
		\caption{With Increased Process Uncertainity}
	\end{subfigure}
	\begin{subfigure}[t]{0.32\textwidth}
		\includegraphics[width=\linewidth]{../thesis_ws/images/uncertainity_in_robot_position_lidar.png}
		\caption{With Increased Lidar uncertainity}
	\end{subfigure}
	\caption{EKF position covariance across submaps, showing variance in x and y direction}
	\label{fig:Variance}
\end{figure}

Overall, the uncertainty plots shown in figure$~$\ref{fig:Variance} reflect the expected sensitivity to the noise models. In the baseline case, the variance remains low, peaking near $0.007$, indicating stable confidence under nominal assumptions. When process uncertainty is increased, the predicted covariance inflates substantially and the peak reaches about $0.12$, showing that the filter quickly becomes less confident in its motion model. In the high LiDAR‑uncertainty case, the variance peaks around $0.01$ and decays more slowly, because measurements are de‑weighted and the Kalman gain is reduced, so the filter takes longer to recover confidence. The initial peak in the middle submaps aligns with the featureless corridor, where limited geometric constraints reduce observability and further slow uncertainty reduction.  In that corridor, uncertainty in $x$ exceeds $y$ because motion and drift are less constrained along the corridor axis, while the lateral direction remains better observed. 
...
\begin{figure}[htbp]
	\centering
	\begin{subfigure}[t]{0.32\textwidth}
		\includegraphics[width=\linewidth]{../thesis_ws/images/submap_confidence_2.png}
		\caption{Baseline process and lidar noise}
	\end{subfigure}
	\begin{subfigure}[t]{0.32\textwidth}
		\includegraphics[width=\linewidth]{../thesis_ws/images/submap_confidence_odom.png}
		\caption{With Increased Process Uncertainity}
	\end{subfigure}
	\begin{subfigure}[t]{0.32\textwidth}
		\includegraphics[width=\linewidth]{../thesis_ws/images/submap_confidence_lidar.png}
		\caption{With Increased Lidar uncertainity}
	\end{subfigure}
	\caption{Confidence value for each submap generated for all 3 cases}
...
Figure~\ref{fig:submap_confidence} summarizes how the EKF confidence score changes across submaps for the three noise settings. In the baseline case, confidence rises rapidly and stays near its ceiling, indicating stable estimation under nominal assumptions. When process uncertainty is increased, confidence becomes more volatile and drops sharply in later submaps, which is consistent with a less trusted motion model and a larger predicted covariance. When LiDAR uncertainty is increased, confidence grows more slowly and levels off below the baseline, reflecting reduced measurement influence and more conservative updates.

From the plot, the baseline confidence stays roughly between \(0.7\) and \(1.0\), with most points near \(0.9\). 
With increased process uncertainty, confidence spans about \(0.3\) to \(1.0\) and drops to around \(0.3\) near the end. 
With increased LiDAR uncertainty, confidence ranges from about \(0.1\) to \(0.95\), rises gradually, and then fluctuates around \(0.7\)–\(0.9\).
...
\begin{thebibliography}{9}
	...
	``Past, Present, and Future of Simultaneous Localization And Mapping: Towards the
	Robust-Perception Age''
	\emph{IEEE Transactions on Robotics},
	vol.~32, no.~6, pp.~1309-1332, 2016.
	
	\bibitem{yue2023lidar_slam_survey}
	Yongjun Zhang, Pengcheng Shi, Jiayuan Li,
	``3D LiDAR SLAM: A survey'',
	\emph{Remote Sensing and Photogrammetry Society and John Wiley \& Sons Ltd},
	...
	14(2):239–256,
	1992. doi: 10.1109/34.121791.
	
	
	\bibitem{censi}
	Andrea Censi, 
	``An accurate closed-form estimate of ICP’s covariance,''
	\emph{ Proceedings 2007 IEEE International Conference on Robotics and Automation}, 
	2007.
	...
	\emph{Technical Report CMU-RI-TR-92-01},
	...
	\bibitem{Ince}
	Ibrahim Ince , Derya Yiltas-Kaplan, and Fatih Kele¸s,
	``From Simulation to Reality: Comparative Performance Analysis
	of SLAM Toolbox and Cartographer in ROS 2``
	\emph{Electronics},
	doi.org/10.3390/electronics14244822
